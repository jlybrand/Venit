...
const MqSearch = {
  ...
  fetchData: async function (params) {
    try {
      ...
      if (apiResponse.data.resultsCount) {
        const searchResults =
          apiResponse.data.searchResults.map(result => {
  ...
  filterAndSetProspect: function (origin, results) {
    for (let i = 0; i < results.length; i++) {
      if (origin.includes(results[i].address) &&
  ...
  saveProspect: async function () {
    ...
    try {
      ...
      if (data.rowCount != 0) {
        ...
        prospect.phone =
          utils.formatPhoneNumber(prospect.phone);
  ...
  getProspect() { return this._prospect; },

  setProspect(value) { this._prospect = value; },
  ...
  setBusinessType(sicCode) { this._businessType = businessTypes[sicCode]; },

  getEligibleResults() { return this._eligibleResults; },

  setEligibleResults(value) { this._eligibleResults = value; },
